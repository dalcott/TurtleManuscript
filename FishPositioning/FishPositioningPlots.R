...
Fish = read_xlsx("../VideoObservations/VideoBehaviorsDatasheet.xlsx") # information regarding each obseration including if fish was attacked, if attack was successful, etc.
...
p1_n = PlotData %>% 
  filter(Species == "RH" & TurtleAvailable == "NoTurtle") %>%
...
p2_n = PlotData %>% 
  filter(Species=="RH" & TurtleAvailable=="Turtle") %>%
  tally() # number of records in this group
...
p2 = ggplot(subset(PlotData, Species=="RH" & TurtleAvailable=="Turtle"), aes(x=X, y=Y, colour = CatchSuccess1)) + 
  geom_point(alpha=0.5, size=2)+
  scale_colour_manual(values = MyPal, drop = FALSE,
                      breaks = c("NoAttack", "AttackMissed", "FishCaptured"),
                      labels = c("No Attack", "Attack Missed", "Fish Captured"))+
...
#ggsave('Plots/PositionPlotWithLegend.png', p2, width = 6, height = 5, units = "in", dpi = 500) # saved to get a legend to include in final 4-panel plot
...
p3 = ggplot(subset(PlotData, Species!="RH" & TurtleAvailable=="NoTurtle"), aes(x=X, y=Y, colour = CatchSuccess1)) + 
  geom_point(alpha=0.5, size=2)+
  scale_colour_manual(values = MyPal, drop = FALSE,
                      breaks = c("NoAttack", "AttackMissed", "FishCaptured"),
                      labels = c("No Attack", "Attack Missed", "Fish Captured"))+
...
p4 = ggplot(subset(PlotData, Species!="RH" & TurtleAvailable=="Turtle"), aes(x=X, y=Y, colour = CatchSuccess1)) + 
  geom_point(alpha=0.5, size=2)+
  scale_colour_manual(values = MyPal, drop = FALSE,
                      breaks = c("NoAttack", "AttackMissed", "FishCaptured"),
                      labels = c("No Attack", "Attack Missed", "Fish Captured"))+
...
# ggplot(subset(NonRH, TurtleAvailable == "NoTurtle"), 
